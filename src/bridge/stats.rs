...
pub fn register_sync<'js>(ctx: &Ctx<'js>, aggregator: SharedAggregator) -> Result<()> {
    ...
    stats.set(
        ...
        Function::new(
            ctx.clone(),
            move |ctx: Ctx<'js>, name: String| -> Result<Object<'js>> {
                let res = Object::new(ctx.clone())?;

                let agg_guard = ctx
                    .userdata::<SharedAggregator>()
                    .expect("Aggregator missing in userdata");
                ...
                if let Ok(agg) = agg.read() {
                    if let Some(s) = agg.requests.get(&name) {
                        ...
                        res.set(
                            "avg",
                            s.total_duration.as_millis() as f64 / s.total_requests.max(1) as f64,
                        )?;
