use rquickjs::{Ctx, Function, Result, Value, JsLifetime, Object, class::Trace};
use std::cell::RefCell;
use std::path::PathBuf;
use prost_reflect::{DescriptorPool, DynamicMessage, MethodDescriptor, FieldDescriptor, Kind, Value as ProstValue};
use tonic::{
    transport::{Channel, Endpoint},
    client::Grpc,
    codec::{Codec, EncodeBuf, DecodeBuf},
    Status, Request,
};
use prost::bytes::Buf; 
...
fn json_to_dynamic_message(desc: prost_reflect::MessageDescriptor, json: serde_json::Value) -> Result<DynamicMessage> {
    ...
    if let serde_json::Value::Object(map) = json {
        ...
        return Err(rquickjs::Error::new_from_js("Expected JSON object for message", "TypeError"));
    }
...
fn json_val_to_prost(json: serde_json::Value, field: &FieldDescriptor) -> Result<ProstValue> {
    match field.kind() {
        Kind::Double => json.as_f64().map(ProstValue::F64).ok_or_else(|| rquickjs::Error::new_from_js("Expected double", "TypeError")),
        Kind::Float => json.as_f64().map(|f| ProstValue::F32(f as f32)).ok_or_else(|| rquickjs::Error::new_from_js("Expected float", "TypeError")),
        Kind::Int32 | Kind::Sint32 | Kind::Sfixed32 => json.as_i64().map(|i| ProstValue::I32(i as i32)).ok_or_else(|| rquickjs::Error::new_from_js("Expected int32", "TypeError")),
        Kind::Int64 | Kind::Sint64 | Kind::Sfixed64 => json.as_i64().map(ProstValue::I64).ok_or_else(|| rquickjs::Error::new_from_js("Expected int64", "TypeError")),
        Kind::Uint32 | Kind::Fixed32 => json.as_u64().map(|i| ProstValue::U32(i as u32)).ok_or_else(|| rquickjs::Error::new_from_js("Expected uint32", "TypeError")),
        Kind::Uint64 | Kind::Fixed64 => json.as_u64().map(ProstValue::U64).ok_or_else(|| rquickjs::Error::new_from_js("Expected uint64", "TypeError")),
        Kind::Bool => json.as_bool().map(ProstValue::Bool).ok_or_else(|| rquickjs::Error::new_from_js("Expected bool", "TypeError")),
        Kind::String => json.as_str().map(|s| ProstValue::String(s.to_string())).ok_or_else(|| rquickjs::Error::new_from_js("Expected string", "TypeError")),
        ...
        _ => Err(rquickjs::Error::new_from_js("Unsupported field type in JSON conversion (TODO: complete mapping)", "TypeError")),
    }
}
...
impl Codec for DynamicCodec {
    ...
    fn encoder(&mut self) -> Self::Encoder {
        DynamicEncoder { method: self.method.clone() }
    }

    fn decoder(&mut self) -> Self::Decoder {
        DynamicDecoder { method: self.method.clone() }
    }
}
...
impl tonic::codec::Decoder for DynamicDecoder {
    ...
    fn decode(&mut self, buf: &mut DecodeBuf<'_>) -> std::result::Result<Option<Self::Item>, Self::Error> {
        ...
        msg.merge(buf).map_err(|e| Status::internal(e.to_string()))?;
        Ok(Some(msg))
    }
}
...
impl GrpcClient {
    ...
    pub fn load(&self, files: Vec<String>, includes: Vec<String>) -> Result<()> {
        let file_descriptors = protox::compile(files.iter().map(PathBuf::from), includes.iter().map(PathBuf::from))
            .map_err(|e| {
    ...
    pub async fn invoke<'js>(&self, ctx: Ctx<'js>, method_name: String, request_val: Value<'js>) -> Result<Value<'js>> {
        // 1. Get Pool & Method Descriptor
        let (method_desc, service_name, method_short_name) = {
            let pool_borrow = self.pool.borrow();
            let pool = pool_borrow.as_ref().ok_or_else(|| rquickjs::Error::new_from_js("Proto not loaded", "StateError"))?;
            ...
            let (service_name, method_short_name) = match clean_name.rsplit_once('/') {
                ...
                None => match clean_name.rsplit_once('.') {
                    Some(v) => v,
                    None => return Err(rquickjs::Error::new_from_js("Invalid method format", "FormatError"))
                }
            };
            ...
            let method_desc = service.methods().find(|m| m.name() == method_short_name)
                .ok_or_else(|| rquickjs::Error::new_from_js("Method not found", "NotFoundError"))?;
            
            (method_desc, service_name.to_string(), method_short_name.to_string())
        };
        ...
        let json_obj: serde_json::Value = if request_val.is_object() {
            ...
            return Err(rquickjs::Error::new_from_js("Request body must be object", "TypeError"));
        };
        ...
        let channel = {
            ...
            channel_borrow.as_ref().ok_or_else(|| rquickjs::Error::new_from_js("Not connected", "StateError"))?.clone()
        };
        ...
        let path_uri: http::Uri = path.parse().map_err(|_| rquickjs::Error::new_from_js("Invalid URI path", "PathError"))?;
        let path_http = http::uri::PathAndQuery::try_from(path_uri.path()).map_err(|_| rquickjs::Error::new_from_js("Invalid PathAndQuery", "PathError"))?;

        let codec = DynamicCodec::new(method_desc.clone());
        
        let request = Request::new(request_msg);
        let response = grpc.unary(request, path_http, codec).await
            .map_err(|e| {
