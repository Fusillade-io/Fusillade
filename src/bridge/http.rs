...
impl<'js> IntoJs<'js> for HttpResponse {
    fn into_js(self, ctx: &Ctx<'js>) -> Result<Value<'js>> {
        ...
        timings_obj.set("tls_handshaking", self.timings.tls_handshaking.as_secs_f64() * 1000.0)?;
...
pub fn register_sync(ctx: &Ctx, tx: Sender<Metric>, client: HttpClient, runtime: Arc<Runtime>, jitter_str: Option<String>, drop_prob: Option<f64>, response_sink: bool) -> Result<()> {
        ...
        loop {
            ...
            let mut cookie_header_val = String::new();
            {
                ...
                for (name, value) in store.get_request_values(&url_parsed) {
                    if !cookie_header_val.is_empty() { cookie_header_val.push_str("; "); }
                    cookie_header_val.push_str(name);
                    cookie_header_val.push('=');
                    cookie_header_val.push_str(value);
                }
            }
            if !cookie_header_val.is_empty() { req_builder = req_builder.header("Cookie", cookie_header_val); }

            // Headers
            if let Some(h) = &headers_map {
                for (k, v) in h { req_builder = req_builder.header(k, v); }
            }
            ...
            let req = req_builder.body(final_body).map_err(|_| rquickjs::Error::Exception)?;
            ...
            let res_result = runtime.block_on(async {
                ...
                if let Some(d) = drop {
                    if rand::thread_rng().gen::<f64>() < d {
                        return Err(Box::new(std::io::Error::new(std::io::ErrorKind::ConnectionReset, "Simulated network drop")) as Box<dyn std::error::Error + Send + Sync>);
                    }
                }
                // Chaos: Jitter
                ...
                if let Some(timeout) = timeout_duration {
                    match tokio::time::timeout(timeout, request_future).await {
                        Ok(result) => result,
                        Err(_) => Err(Box::new(std::io::Error::new(std::io::ErrorKind::TimedOut, "request timeout")) as Box<dyn std::error::Error + Send + Sync>),
                    }
                } else {
            ...
            match res_result {
                Ok((response, timings)) => {
                    ...
                    for (name, val) in response.headers() {
                        if let Ok(val_str) = val.to_str() {
                            ...
                            if lower == "set-cookie" { set_cookie_headers.push(val_str.to_string()); }
                            if lower == "location" { location_header = Some(val_str.to_string()); }
                        }
                    }
                ...
                Err(e) => {
                    ...
                    return Ok(HttpResponse { status: 0, body: Bytes::from(e.to_string()), headers: HashMap::new(), timings, proto: "h1".to_string() });
                }
            }
        }
    }))?;
    ...
    http.set("get", Function::new(ctx.clone(), move |url_str: String, rest: rquickjs::function::Rest<rquickjs::Value>| -> Result<HttpResponse> {
        ...
        let tags: HashMap<String, String> = if let Some(arg) = rest.first() {
            arg.as_object().and_then(|o| o.get("tags").ok()).unwrap_or_default()
        } else {
            HashMap::new()
        };
        ...
        loop {
            let url_parsed = Url::parse(&current_url).map_err(|_| rquickjs::Error::new_from_js("Invalid URL", "UrlError"))?;
            ...
            let req = req_builder.body(String::new()).map_err(|_| rquickjs::Error::Exception)?;
            ...
            let res_result = runtime.block_on(async {
                if let Some(d) = drop {
                     if rand::thread_rng().gen::<f64>() < d { return Err(Box::new(std::io::Error::new(std::io::ErrorKind::ConnectionReset, "Simulated network drop")) as Box<dyn std::error::Error + Send + Sync>); }
                }
            ...
            match res_result {
                Ok((response, timings)) => {
                    ...
                    let metric_name = name_tag.clone().unwrap_or_else(|| current_url.clone());
                    let _ = tx.send(Metric::Request {
                        name: format!("{}{}", crate::bridge::group::get_current_group_prefix(), metric_name),
                        timings,
                        status,
        ...
        if is_multipart {
            ...
            if let Ok(form_data) = serde_json::from_str::<serde_json::Value>(&body) {
                if let Some(obj) = form_data.as_object() {
                    ...
                    let blocking_client = client_builder.build().unwrap_or_else(|_| reqwest::blocking::Client::new());
                    let mut form = reqwest::blocking::multipart::Form::new();
                    
                    for (key, value) in obj {
                        if let Some(file_obj) = value.as_object() {
                            if file_obj.get("__fusillade_file").and_then(|v| v.as_bool()).unwrap_or(false) {
                                // This is a file field
                                let data_b64 = file_obj.get("data").and_then(|v| v.as_str()).unwrap_or("");
                                let filename = file_obj.get("filename").and_then(|v| v.as_str()).unwrap_or("file");
                                let content_type = file_obj.get("contentType").and_then(|v| v.as_str()).unwrap_or("application/octet-stream");
                                ...
                                if let Ok(data) = base64::engine::general_purpose::STANDARD.decode(data_b64) {
                                    let part = reqwest::blocking::multipart::Part::bytes(data)
                                        ...
                                        .mime_str(content_type).unwrap_or_else(|_| 
                    ...
                    if let Some(d) = drop {
                         if rand::thread_rng().gen::<f64>() < d { return Err(rquickjs::Error::new_from_js("Simulated network drop", "NetworkError")); }
                    }
                    if let Some(j) = jitter { std::thread::sleep(j); }
                    ...
                    match res {
                        Ok(response) => {
                            ...
                            return Ok(HttpResponse { status, body: Bytes::from(body_bytes.to_vec()), headers: resp_headers, timings, proto });
                        },
                        Err(e) => {
                            let timings = RequestTimings { duration, ..Default::default() };
                            let metric_name = name_tag.clone().unwrap_or_else(|| current_url.clone());
            ...
            match res_result {
                Ok((response, timings)) => {
...

                    {
                        ...
                        for cookie_val in set_cookie_headers { let _ = store.parse(&cookie_val, &url_parsed); }
                    }
    ...
    http.set("put", Function::new(ctx.clone(), move |url_str: String, body: String, rest: rquickjs::function::Rest<rquickjs::Value>| -> Result<HttpResponse> {
        ...
        let mut cookie_header_val = String::new();
        { let store = cs_put.borrow(); for (name, value) in store.get_request_values(&url_parsed) { if !cookie_header_val.is_empty() { cookie_header_val.push_str("; "); } cookie_header_val.push_str(name); cookie_header_val.push('='); cookie_header_val.push_str(value); } }
        if !cookie_header_val.is_empty() { req_builder = req_builder.header("Cookie", cookie_header_val); }
        if let Some(h) = &headers_map { for (k, v) in h { req_builder = req_builder.header(k, v); } }
        let req = req_builder.body(body).map_err(|_| rquickjs::Error::Exception)?;
        let start = Instant::now();
        // Execute request with optional timeout
        let res_result = runtime.block_on(async {
        ...
        match res_result {
            Ok((response, timings)) => {
                ...
                let proto = version_to_proto(response.version());
                let mut resp_headers: HashMap<String, String> = HashMap::with_capacity(response.headers().len());
                for (name, val) in response.headers() { if let Ok(val_str) = val.to_str() { resp_headers.insert(name.to_string(), val_str.to_string()); } }
                let body = response.body().clone();
                let metric_name = name_tag.clone().unwrap_or_else(|| url_str.clone());
                let _ = tx.send(Metric::Request { name: format!("{}{}", crate::bridge::group::get_current_group_prefix(), metric_name), timings, status, error: None, tags: tags.clone() });
                Ok(HttpResponse { status, body, headers: resp_headers, timings, proto })
            },
            Err(e) => {
                let timings = RequestTimings { duration: start.elapsed(), ..Default::default() };
                let metric_name = name_tag.clone().unwrap_or_else(|| url_str.clone());
                let _ = tx.send(Metric::Request { name: format!("{}{}", crate::bridge::group::get_current_group_prefix(), metric_name), timings, status: 0, error: Some(e.to_string()), tags: tags.clone() });
                Ok(HttpResponse { status: 0, body: Bytes::from(e.to_string()), headers: HashMap::new(), timings, proto: "h1".to_string() })
            }
        }
    }))?;
    ...
    http.set("del", Function::new(ctx.clone(), move |url_str: String, rest: rquickjs::function::Rest<rquickjs::Value>| -> Result<HttpResponse> {
        ...
        let mut cookie_header_val = String::new();
        { let store = cs_del.borrow(); for (name, value) in store.get_request_values(&url_parsed) { if !cookie_header_val.is_empty() { cookie_header_val.push_str("; "); } cookie_header_val.push_str(name); cookie_header_val.push('='); cookie_header_val.push_str(value); } }
        if !cookie_header_val.is_empty() { req_builder = req_builder.header("Cookie", cookie_header_val); }
        if let Some(h) = &headers_map { for (k, v) in h { req_builder = req_builder.header(k, v); } }
        let req = req_builder.body(String::new()).map_err(|_| rquickjs::Error::Exception)?;
        let start = Instant::now();
    ...
    http.set("patch", Function::new(ctx.clone(), move |url_str: String, body: String, rest: rquickjs::function::Rest<rquickjs::Value>| -> Result<HttpResponse> {
        ...
        let mut cookie_header_val = String::new();
        { let store = cs_patch.borrow(); for (name, value) in store.get_request_values(&url_parsed) { if !cookie_header_val.is_empty() { cookie_header_val.push_str("; "); } cookie_header_val.push_str(name); cookie_header_val.push('='); cookie_header_val.push_str(value); } }
        if !cookie_header_val.is_empty() { req_builder = req_builder.header("Cookie", cookie_header_val); }
        if let Some(h) = &headers_map { for (k, v) in h { req_builder = req_builder.header(k, v); } }
        let req = req_builder.body(body).map_err(|_| rquickjs::Error::Exception)?;
        let start = Instant::now();
    ...
    http.set("head", Function::new(ctx.clone(), move |url_str: String, rest: rquickjs::function::Rest<rquickjs::Value>| -> Result<HttpResponse> {
        ...
        let mut cookie_header_val = String::new();
        { let store = cs_head.borrow(); for (name, value) in store.get_request_values(&url_parsed) { if !cookie_header_val.is_empty() { cookie_header_val.push_str("; "); } cookie_header_val.push_str(name); cookie_header_val.push('='); cookie_header_val.push_str(value); } }
        if !cookie_header_val.is_empty() { req_builder = req_builder.header("Cookie", cookie_header_val); }
        let req = req_builder.body(String::new()).map_err(|_| rquickjs::Error::Exception)?;
        let start = Instant::now();
        // Execute request with optional timeout
        ...
        match res_result {
            Ok((response, timings)) => {
                ...
                let metric_name = name_tag.clone().unwrap_or_else(|| url_str.clone());
                let _ = tx.send(Metric::Request { name: format!("{}{}", crate::bridge::group::get_current_group_prefix(), metric_name), timings, status, error: None, tags: tags.clone() });
                Ok(HttpResponse { status, body: Bytes::new(), headers: resp_headers, timings, proto })
            },
            Err(e) => {
                let timings = RequestTimings { duration: start.elapsed(), ..Default::default() };
                let metric_name = name_tag.clone().unwrap_or_else(|| url_str.clone());
    ...
    http.set("options", Function::new(ctx.clone(), move |url_str: String, rest: rquickjs::function::Rest<rquickjs::Value>| -> Result<HttpResponse> {
        ...
        let mut cookie_header_val = String::new();
        { let store = cs_opts.borrow(); for (name, value) in store.get_request_values(&url_parsed) { if !cookie_header_val.is_empty() { cookie_header_val.push_str("; "); } cookie_header_val.push_str(name); cookie_header_val.push('='); cookie_header_val.push_str(value); } }
        if !cookie_header_val.is_empty() { req_builder = req_builder.header("Cookie", cookie_header_val); }
        let req = req_builder.body(String::new()).map_err(|_| rquickjs::Error::Exception)?;
        let start = Instant::now();
        // Execute request with optional timeout
    ...
    http.set("file", Function::new(ctx.clone(), move |path: String, filename: Option<String>, content_type: Option<String>| -> Result<String> {
        ...
        let data = std::fs::read(&file_path).map_err(|_| {
            rquickjs::Error::new_from_js("IOError", "Failed to read file")
        })?;
