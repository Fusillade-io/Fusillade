...
mod tests {
    ...
    fn test_send_histogram() {
        let (tx, rx) = unbounded();
        send_histogram(&tx, "response_time".to_string(), 123.45);
    ...
    fn test_send_counter() {
        let (tx, rx) = unbounded();
        send_counter(&tx, "requests".to_string(), 1.0);
    ...
    fn test_send_gauge() {
        let (tx, rx) = unbounded();
        send_gauge(&tx, "active_users".to_string(), 42.0);
    ...
    fn test_send_rate_success() {
        let (tx, rx) = unbounded();
        send_rate(&tx, "check_passed".to_string(), true);
    ...
    fn test_send_rate_failure() {
        let (tx, rx) = unbounded();
        send_rate(&tx, "check_failed".to_string(), false);
    ...
    fn test_multiple_metrics() {
        ...
        send_histogram(&tx, "hist1".to_string(), 1.0);
        send_counter(&tx, "counter1".to_string(), 2.0);
        send_gauge(&tx, "gauge1".to_string(), 3.0);
        send_rate(&tx, "rate1".to_string(), true);
    ...
    fn test_histogram_negative_value() {
        let (tx, rx) = unbounded();
        send_histogram(&tx, "temp".to_string(), -10.5);
    ...
    fn test_counter_large_value() {
        let (tx, rx) = unbounded();
        send_counter(&tx, "bytes".to_string(), 1_000_000_000.0);
