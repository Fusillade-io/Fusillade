...
pub fn generate_html(report: &ReportStats) -> String {
    ...
    html.push_str(&format!("<p>Average Latency: {:.2} ms</p>", report.avg_latency_ms));
    html.push_str(&format!("<p>P95 Latency: {:.2} ms</p>", report.p95_latency_ms));
    html.push_str(&format!("<p>P99 Latency: {:.2} ms</p>", report.p99_latency_ms));
